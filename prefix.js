...
document.getElementById('conversionType').addEventListener('change', function () {
  ...
  const processInput = (char, expression, index) => {
    ...
    if (/[a-z0-9]/i.test(char)) {
      ...
      const popedElement = operatorStack.pop();
      setTimeout(()=>{updateStackView();},1000);
      displayMessage(`Step ${index+1}:   ${popedElement} is poped from stack`);
  ...
  push.addEventListener("click", () => {
    ...
    setTimeout(() => {
      while (operatorStack?.length > 0) {
        ...
        displayMessage(`Step ${stepIndex + 1}: ${popedElement} is popped from the stack`);
        stepIndex++;
      }
      boxPostfix.innerHTML = prefixExpression;
      setTimeout(()=>{
      ...
      setTimeout(()=>{
        ...
        divElements.forEach(span => {
          ...
          setTimeout(()=>{
              ...
              boxPostfix.innerHTML = prefixExpression;
              setTimeout(()=>{displayMessage("Expression is Evaluated");},1000);
            },2000);
      });
